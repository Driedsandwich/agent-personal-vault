...
Use the GitHub README as the primary announcement link. PyPI latest is `0.1.18`, the `v0.1.18` GitHub prerelease is published, and Trusted Publishing OIDC publish has succeeded for `v0.1.5` through `v0.1.18`. The PyPI long description matches the current README, but generic announcement text should still avoid quoting a pinned `pip install agent-personal-vault==...` command. If a channel requires install instructions, link to the current README instead of copying a version-pinned command into the post.
...
- Optional secondary link after the main text: GitHub release `v0.1.18` if the post is explicitly about the current alpha release.
