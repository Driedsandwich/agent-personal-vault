...
Decision: keep the project in public alpha / prerelease status after the approved `v0.1.14` GitHub prerelease and Trusted Publishing OIDC PyPI publish. Do not create another release, tag, package publish, repository setting change, branch deletion, or announcement without a separate approval lane.
...
- Post-release checks for `v0.1.14` confirmed PyPI availability, installability, console script availability, Project-URL metadata, PyPI provenance/attestations, GitHub Actions publish success, and open security alert count 0.
...
Post-`v0.1.14` status sync evidence:

- GitHub release `v0.1.14` is a published prerelease, not a draft, and points to `43ca9aa768c7453fb34846655bb4539086fb7582`.
- PyPI latest is `0.1.14`; the wheel and sdist are present with SHA-256 `3f8db34f339b98f830ac8cda6f193d4465a6f4a3d56577c043edba3e0bfd56b8` and `3b7f659cba32b69ee789ac54aa60dae1c0501ddb8769629876e2b327cab78aa8`.
- PyPI Project-URL metadata contains Homepage, Source, Issues, and Documentation links to the GitHub repository and README.
- A clean temporary `pip install agent-personal-vault==0.1.14` confirmed installed metadata version `0.1.14`, and the `agent-personal-vault`, `apv-gui`, and `apv-mcp` console scripts were available.
- The PyPI long description contains `agent-personal-vault==0.1.14` and no longer contains `agent-personal-vault==0.1.13`.
- PyPI integrity provenance endpoints returned attestation data for both the `0.1.14` wheel and sdist.
- GitHub Actions `publish-package` for `v0.1.14`, main `test`, main CodeQL, and dependency graph update completed successfully.
- Open security alerts were 0 for CodeQL, Dependabot, and secret scanning during this status sync.
- Open GitHub state during this status sync was Issue #231 only and no open PRs.
...
This historical checklist records pre-`v0.1.0` release-candidate preparation. It is not the current package state. Current state is `v0.1.14` GitHub prerelease and PyPI `0.1.14`, with Trusted Publishing OIDC validated.
...
- Historical version candidate: this section used `0.1.0` as the then-current alpha package version. Do not treat that as current; latest PyPI is `0.1.14`.
...
- Historical version candidate: `0.1.0`. Latest PyPI package is now `0.1.14`.
...
- Historical repository snapshot: releases 0, tags 0, open PRs 0. The only open Issue during this dry-run was the tracking Issue for that work. This is no longer current; the project now has published prereleases and tags through `v0.1.14`.
...
- Historical target version: `0.1.0` for the first public-alpha line. Latest PyPI package is now `0.1.14`.
...
This section records the historical `v0.1.4` candidate and publish decision. It is retained as release evidence only. It is not the current package state; latest GitHub prerelease and PyPI package are `v0.1.14`.
