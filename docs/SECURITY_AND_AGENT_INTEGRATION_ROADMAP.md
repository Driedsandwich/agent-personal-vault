...
| MCP権限管理 | 実装する | P1 | 汎用AIエージェント連携には有効。ただし監査ログ・同意UIより後。 |
...
```json
{
  ...
  "require_consent": ["get", "env", "set", "unset"],
...
- purposeやconsentがまだ構造化されていない。
- 複数エージェント間の権限差を表現しにくい。

改善:

- `get` に `--purpose` を追加する。
- raw取得はstderr警告、audit記録、必要ならinteractive confirmを挟む。
- `context --task <task>` で、必要候補keyだけをraw-freeで返す将来拡張を検討する。
...
- 最初のMCP版はread-only raw-free toolsだけに限定する。
- raw-returning toolsは監査ログと同意UIが入ってから公開する。
- MCP serverの設定に `raw_tools_enabled: false` を既定値として置く。
...
Status: consent request queue and approve/deny are implemented. Dedicated audit viewer remains pending.
...
1. GUI audit viewerを追加する。
2. consent tokenの有効期限、使用済み状態、拒否履歴をGUIで表示する。
3. testsで「GUI同意画面がraw値を表示しない」ことを確認する。
