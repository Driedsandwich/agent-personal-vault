...
Use PyPI Trusted Publishing as the normal package publish path after successful OIDC publishes from `v0.1.5` through `v0.1.17`.
...
- Trusted Publishing has been used successfully for `v0.1.5` through `v0.1.17`.
...
The workflow file exists and OIDC publishes succeeded for `v0.1.5` through `v0.1.17`. Do not run it for another real publish until the next version, tag, release, package availability, artifact, CI, security, and human approval gates are complete.
...
The first OIDC publish was completed for `v0.1.5`, and the same lane was reused successfully through `v0.1.17`. Future OIDC publishes should remain separate release lanes for approved versions, not background migration steps.
